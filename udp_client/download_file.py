...
def download_file(server_address, name, dst):
  ...
  response = send_message('download' + DELIMITER + name, server_address, client_socket)
  ...
  send_message('start transmission', server_address, client_socket)
...
def recv_file(file_size, chunks, offset):
  ...
  received_chunks = (-1, -1) #ultimo recibido en orden, ultimo recibido en general
  ...
  while total_received_size < file_size: # while no es fin or timeout
    ...
    if expected_offset_number != offset_number:
      client_socket.sendto(expected_offset_number.encode(), server_address)
    else:
